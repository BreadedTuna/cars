...
    // 1. Mobile Check: Always use Redirect for mobile devices (most reliable)
...
        
        // If popup is blocked, try redirect as a single, clean fallback
...
      });
  } // <-- NO extra code or functions immediately following this brace!
...
        // Authorization check is handled by onAuthStateChanged
      }
    })
    .catch(function (error) {
      log("getRedirectResult error:", error);
      // Start email fallback only if the redirect result itself failed.
